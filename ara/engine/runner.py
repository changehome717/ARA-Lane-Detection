...
from ara.models.registry import build_net
from .registry import build_trainer, build_evaluator
from .optimizer import build_optimizer
from .scheduler import build_scheduler
from ara.datasets import build_dataloader
from ara.utils.recorder import build_recorder
from ara.utils.net_utils import save_model, load_network, resume_network
...
class Runner(object):
    ...
    def test(self):
        ...
        for _, data in enumerate(tqdm(self.test_loader, desc='Testing')):
            data = self.to_cuda(data)
            with torch.no_grad():
                output = self.net(data)
    ...
    def validate(self):
        ...
        for _, data in enumerate(tqdm(self.val_loader, desc='Validate')):
            data = self.to_cuda(data)
            with torch.no_grad():
                output = self.net(data)
                output = self.net.module.heads.get_lanes(output)
