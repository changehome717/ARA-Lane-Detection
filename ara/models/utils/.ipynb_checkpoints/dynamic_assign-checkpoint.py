import torch
from ara.models.losses.talat_loss import line_iou
...
def focal_cost(cls_pred, gt_labels, alpha=0.25, gamma=2, eps=1e-12):
    ...
    Args:
        ...
        gt_labels (Tensor): label of gt, shape (num_gt,)
    Returns:
        Tensor: cls_cost
    """
...
def dynamic_k_assign(cost, pair_wise_ious):
    """
    Assign ground truths with priors dynamically.
    Returns:
        prior_idx, gt_idx
    """
...
def assign(predictions,
           ...
           cls_cost_weight=1.0):
    """
    Dynamic matching based on cost (cls + similarity).
    Args:
        predictions: (num_priors, 78)
        targets: (num_targets, 78)
    Returns:
    ...
    theta_score = torch.cdist(predictions[:, 4].unsqueeze(-1), target_thetas, p=1).reshape(num_priors, num_targets) * 180
    theta_score = (1 - theta_score / torch.max(theta_score)) + 1e-2

    cost = -(distances_score * start_xys_score * theta_score)**2 * distance_cost_weight + cls_score * cls_cost_weight

    iou, _, _ = line_iou(predictions[..., 6:], targets[..., 6:], img_w, aligned=False)
