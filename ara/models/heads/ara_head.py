...
from ara.utils.lane import Lane
from ara.models.losses.focal_loss import FocalLoss
from ara.models.losses.accuracy import accuracy
from ara.ops import nms

from ara.models.utils.roi_gather import ROIGather, LinearModule
from ara.models.utils.seg_decoder import SegDecoder
from ara.models.utils.dynamic_assign import assign
from ara.models.losses.talat_loss import talat_loss
from ..registry import HEADS


@HEADS.register_module
class AraLaneHead(nn.Module):
    def __init__(self,
                 num_points=64,
                 ...
                 sample_points=36,
                 cfg=None):
        super(AraLaneHead, self).__init__()
        self.cfg = cfg
        ...
        self.register_buffer(
            ...
            tensor=(torch.linspace(0, 1, steps=self.sample_points, dtype=torch.float32) *
                    self.n_strips).long()
        )
        self.register_buffer(
            name='prior_feat_ys',
    ...
    def pool_prior_features(self, batch_features, num_priors, prior_xs):
        ...
        Args:
            batch_features (Tensor): feature map (B, C, H, W)
        """
    ...
    def generate_priors_from_embeddings(self):
        ...
        # 2 scores + [start_y, start_x, theta] + [length, unused] + offsets
    ...
    def _init_prior_embeddings(self):
        # [start_y, start_x, theta] normalized
    ...
    def forward(self, x, **kwargs):
        """
        Iterative refinement over pyramid features and output lane predictions.
        Args:
            x: list[Tensor] feature pyramids
        Returns:
            train: dict(loss, loss_stats, ...)
            eval: Tensor predictions
        """
        ...
        for stage in range(self.refine_layers):
            ...
            fc_features = fc_features.view(num_priors, batch_size, -1).reshape(batch_size * num_priors,
                                                                               self.fc_hidden_dim)
    ...
    def predictions_to_pred(self, predictions):
        ...
        for lane in predictions:
            ...
            lane_obj = Lane(points=points.cpu().numpy(),
    ...
    def loss(self,
             ...
             seg_loss_weight=1.,
             ang_loss_weight=1.):
        # read weights from cfg if present
        ...
        if self.cfg.haskey('ang_loss_weight'):
            ang_loss_weight = self.cfg.ang_loss_weight
        ...
        for stage in range(self.refine_layers):
            ...
            for predictions, target in zip(predictions_list, targets):
                ...
                target_yxtl[:, 2] *= 180
                reg_xytl_loss = reg_xytl_loss + F.smooth_l1_loss(reg_yxtl, target_yxtl,
                                                                 reduction='none').mean()

                _, iou_assign_loss_one, anchor_angle_diff_one = talat_loss(
                    reg_pred, reg_targets, self.img_w, width=30, aligned=True
                )
                iou_loss += iou_assign_loss_one
                ang_loss += anchor_angle_diff_one
        ...
        denom = (len(targets) * self.refine_layers)
        cls_loss /= denom
        reg_xytl_loss /= denom
        iou_loss /= denom
        ang_loss /= denom
        ...
        loss = (cls_loss * cls_loss_weight +
                ...
                seg_loss * seg_loss_weight +
                iou_loss * iou_loss_weight +
                ang_loss * ang_loss_weight)
        ...
        return_value = {
            ...
            'loss_stats': {
                ...
                'ang_loss': ang_loss * ang_loss_weight,
            }
        }
