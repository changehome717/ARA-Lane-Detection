...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        # fix: use sample['img'] instead of undefined img
        ...
        if random.random() < 0.5:
            ...
            for img, a in zip(img_group, self.applied):
                ...
                if len(img.shape) > len(out_images[-1].shape):
                    out_images[-1] = out_images[-1][..., np.newaxis]
            return out_images
        return img_group
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        img = sample["img"]
        if len(m) == 1:
            img = (img - np.array(m)) / np.array(s)
        else:
            img = (img - np.array(m)[np.newaxis, np.newaxis, ...]) / np.array(s)[np.newaxis, np.newaxis, ...]
        sample["img"] = img
        return sample


def AraTransforms(img_h, img_w):
    return [
