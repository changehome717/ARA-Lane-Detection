...
from ara.utils.llamas_utils import get_horizontal_values_for_four_lanes
import ara.utils.llamas_metric as llamas_metric
...
class LLAMAS(BaseDataset):
    def __init__(self, data_root, split='train', processes=None, cfg=None):
        ...
        if split != 'test':
            self.labels_dir = os.path.join(self.data_root, SPLIT_DIRECTORIES[split])
    ...
    def get_img_name(self, json_path):
        return (json_path.split('/')[-1]).replace('.json', '_color_rect.png')
    ...
    def load_annotations(self):
        if self.split == 'test':
            ...
            self.data_infos = [{
                ...
                'img_name': os.path.join(TEST_IMGS_DIR, root.split('/')[-1], file),
                'lanes': [],
                'relative_path': os.path.join(root.split('/')[-1], file)
            } for root, _, files in os.walk(imgs_dir) for file in files if file.endswith('.png')]
            self.data_infos = sorted(self.data_infos, key=lambda x: x['img_path'])
            return
        ...
        for json_path in tqdm(json_paths):
            ...
            lanes = [[(x, y) for x, y in zip(lane, range(self.cfg.ori_img_h)) if x >= 0] for lane in lanes]
    ...
    def evaluate(self, predictions, output_basedir):
        ...
        result = llamas_metric.eval_predictions(
            output_basedir,
            self.labels_dir,
