...
import ara.utils.culane_metric as culane_metric
...
class CULane(BaseDataset):
    ...
    def load_annotation(self, line):
        ...
        if len(line) > 1:
            ...
            infos['mask_path'] = os.path.join(self.data_root, mask_line)
        ...
        with open(anno_path, 'r') as anno_file:
            data = [list(map(float, ln.split())) for ln in anno_file.readlines()]
    ...
    def evaluate(self, predictions, output_basedir):
        ...
        # category-wise eval (kept)
        for cate, cate_file in CATEGORYS.items():
            _ = culane_metric.eval_predictions(
                output_basedir,
                self.data_root,
